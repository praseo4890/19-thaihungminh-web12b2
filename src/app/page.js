...
import { Plus, ChevronLeft, ChevronRight, ArrowLeft, ArrowRight, Star, ChevronDown, SlidersHorizontal, Search, User, ShoppingBag } from 'lucide-react';
...
function ProductCard({ product, index }) {
  ...
  return (
    ...
    >
      <div className="relative aspect-[4/5] overflow-hidden rounded-md md:rounded-lg bg-[#f2f2f2] w-full">
        <img
          src={product.image}
        ...
        />
        {/* Cart button - ẩn trên mobile vì không có hover */}
        <div className="cart-btn-wrapper hidden sm:flex">
      ...
      <div className="mt-2 md:mt-4 text-left">
        <h3 className="text-[10px] md:text-[14px] font-bold uppercase tracking-tighter line-clamp-1">{product.tenSP}</h3>
        <p className="hidden md:block text-xs text-gray-400 font-mono mt-1 mb-2 line-clamp-1">{product.moTaNgan}</p>
        <p className="text-[9px] md:text-[13px] font-mono font-semibold text-gray-800 mt-1">{priceDisplay}</p>
      </div>
    </Link>
  );
}
...
// Show products with animation
  useEffect(() => {
    if (isStore) {
      // 1. Reset lại class show để ép các DOM ẩn đi trước
      document.querySelectorAll('.product-item').forEach(el => el.classList.remove('show'));
      
      // 2. Chạy lại animation hiện lên
  ...
  }, [isStore, displayedCount, sortLabel]); 
...
const getSortedProducts = () => {
    ...
    if (sortLabel === 'Theo thứ tự chữ cái, A-Z') {      sorted.sort((a, b) => a.tenSP.toLowerCase().localeCompare(b.tenSP.toLowerCase()));
    } 
    else if (sortLabel === 'Theo thứ tự chữ cái, Z-A') {

      sorted.sort((a, b) => b.tenSP.toLowerCase().localeCompare(a.tenSP.toLowerCase()));
    } 
    else if (sortLabel === 'Giá, thấp đến cao') {
      sorted.sort((a, b) => a.giaSP - b.giaSP);
    } 
    else if (sortLabel === 'Giá, cao đến thấp') {
      sorted.sort((a, b) => b.giaSP - a.giaSP);
    }
  ...
  {/* Black sticky header */}
      <header className="bg-black text-white sticky top-0 z-50">
        <div className="max-w-[1600px] mx-auto px-6 h-20 flex items-center justify-between lg:grid lg:grid-cols-[1fr_auto_1fr]">
          ...
          {/* Nhóm 3: Icons (Ngôn ngữ, Tiền tệ, Search, User, Cart) */}
          <div className="flex items-center justify-end gap-3 xl:gap-5 flex-shrink-0">
            ...
            <div className="relative group hidden xl:block">
              ...
              <div className="absolute right-0 top-full mt-6 w-[260px] bg-black text-white opacity-0 invisible translate-y-2 transition-all duration-300 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 z-50 max-h-60 overflow-y-auto">
            ...
            <div className="relative group hidden xl:block">
              ...
              <div className="absolute right-0 top-full mt-6 w-[140px] bg-black text-white opacity-0 invisible translate-y-2 transition-all duration-300 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 z-50">
                <ul className="py-2 text-[10px]">
            ...
            <Link href="/cart" className="relative cursor-pointer hover:text-gray-400">
              <ShoppingBag size={20} />
              <span className="absolute -top-1 -right-2 bg-red-500 text-[9px] font-bold w-4 h-4 flex items-center justify-center rounded-full text-white">
                0
              </span>
            </Link>
          </div>
    ...
    <div id="main-store" className="min-h-screen flex flex-col bg-white">
      ...
      <header className="bg-black text-white sticky top-0 z-50">
        <div className="max-w-[1600px] mx-auto px-6 h-20 flex items-center justify-between lg:grid lg:grid-cols-[1fr_auto_1fr]">
          ...
          <div className="flex items-center justify-end gap-3 xl:gap-5 flex-shrink-0">
            ...
            <button className="cursor-pointer hover:text-gray-400"><Search size={20} /></button>
            <button className="cursor-pointer hover:text-gray-400"><User size={20} /></button>
            <Link href="/cart" className="relative cursor-pointer hover:text-gray-400 flex items-center">
              <ShoppingBag size={20} />
              <span className="absolute -top-1.5 -right-2 bg-red-500 text-[10px] font-bold w-4 h-4 flex items-center justify-center rounded-full text-white">
                0
              </span>
            </Link>
          </div>
      ...
      <div className="flex flex-1 relative w-full">
        ...
        <div className="flex-1 w-full min-w-0 flex flex-col">
          <main className="w-full max-w-[1500px] mx-auto px-3 md:px-12 py-6 md:py-12 flex flex-col">
            ...
            {/* Sort Bar */}
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6 md:mb-12 border-b border-gray-100 pb-4 gap-4">
              <div className="flex items-center space-x-4 md:space-x-6 w-full md:w-auto">
                ...
                <div className="relative inline-block w-full md:w-auto">
                  <button
                    ...
                    className="border border-black px-3 md:px-6 py-2.5 md:py-3 flex items-center justify-between w-full md:min-w-[260px] cursor-pointer bg-white hover:bg-gray-50 transition relative z-20"
                  >
                    <span className="text-[10px] md:text-[12px] font-mono uppercase tracking-widest text-black truncate">{sortLabel}</span>
                    <ChevronDown size={14} className={`ml-2 md:ml-4 flex-shrink-0 transition-transform ${sortOpen ? 'rotate-180' : ''}`} />
                  </button>
                  {sortOpen && (
                    <div className="absolute left-0 top-full mt-1 w-full bg-white border border-gray-200 shadow-2xl z-[100] font-mono text-[10px] md:text-[13px]">
                      <ul className="py-2">
                        {sortOptions.map(opt => (
                          <li key={opt} onClick={() => { setSortLabel(opt); setSortOpen(false); }}
                            className={`px-3 md:px-6 py-2.5 md:py-3 hover:bg-gray-100 cursor-pointer transition-colors ${sortLabel === opt ? 'font-semibold bg-gray-200 border-l-2 border-blue-600' : 'text-gray-600'}`}>
                            {opt}
                          </li>
                        ))}
                      </ul>
            ...
            {/* Product Grid */}
            <div className="grid grid-cols-3 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-8 w-full">
              {shown.map((p, i) => (
            ...
            <section id="best-sellers-section" className="mt-20 md:mt-32 border-t border-gray-100 pt-16 md:pt-24 pb-16 md:pb-20">
              ...
              <div className="grid grid-cols-4 gap-3 md:gap-8 w-full">
                {bestSellers.map((p, i) => (
                  <ProductCard key={p.maSP} product={p} index={i} />
                ))}
              </div>
